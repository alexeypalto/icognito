package by.hmarka.alexey.incognito.customcontrols;
...
public class CustomTabLayout extends FrameLayout {
    ...
    private void populateFromPagerAdapter() {
        ...
        if (mPagerAdapter != null) {
            ...
            mTabStrip.setTabCount(adapterCount);
            for (int i = 0; i < adapterCount; i++) {
                ...
                addTab(tab.setText(mPagerAdapter.getPageTitle(i)), false);
            }
    ...
    public void removeAllTabs() {
        ...
        for (int i = mTabStrip.getTabChildCount() - 1; i >= 0; i--) {
            removeTabViewAt(i);
        }
    ...
    private void removeTabViewAt(int position) {
        final TabView view = (TabView) mTabStrip.getTabChildAt(position);
        mTabStrip.removeTabViewAt(position);
        if (view != null) {
            view.reset();
        }
        requestLayout();
    ...
    private void addTabView(Tab tab, boolean setSelected) {
        ...
        mTabStrip.addTabView(tabView, createLayoutParamsForTabs());
        if (setSelected) {
...
//        }
    ...
    private void setSelectedTabView(int position) {
        final int tabCount = mTabStrip.getTabChildCount();
        if (position < tabCount && !mTabStrip.getTabChildAt(position).isSelected()) {
            for (int i = 0; i < tabCount; i++) {
                final View child = mTabStrip.getTabChildAt(i);
    ...
    private void setScrollPosition(int position, float positionOffset, boolean updateSelectedText,
        ...
        if (roundedPosition < 0 || roundedPosition >= mTabStrip.getTabChildCount()) {
            return;
        }
...
//     */
    ...
    private class SlidingTabStrip extends RelativeLayout {
        ...
        boolean childrenNeedLayout() {
            for (int i = 0, z = getTabChildCount(); i < z; i++) {
                final View child = getTabChildAt(i);
                if (child.getWidth() <= 0) {
                    return true;
                }
            }
...
//            } else {
        ...
        private void updateIndicatorPosition() {
            final View selectedTitle = getTabChildAt(mSelectedPosition);
            int left, right;
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectionOffset > 0f && mSelectedPosition < getTabChildCount() - 1) {
                    ...
                    right = (int) (mSelectionOffset * (nextTitle.getRight()+layoutOffset) +
...
//            }
            ...
            final View targetView = getTabChildAt(position);
            if (targetView == null) {
            ...
            final int targetRight = targetView.getRight()+ targetLayoutOffset;
            final int startLeft;
